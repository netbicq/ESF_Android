 ...
 * 历史任务单据
 */
public class GetTaskBillsOverModel extends MvpModel<GetTaskBillsOverModel.Request, GetTaskBillsOverModel.Response> {
    @Override
    public void getResponse(Request request, MvpCallback<Response> callback) {
        if (!checkNetWork()) {
            //没有网络的话 就访问本地数据库
            try {
                GetTaskBillsOverModel.Response response = new GetTaskBillsOverModel.Response();
